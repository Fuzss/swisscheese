...
public class MapGenRavineHell extends MapGenRavine {
    ...
    {
        if (this.rand.nextInt(50) == 0 && world.provider.isNether())
        {
            double d0 = (double)(chunkX * 16 + this.rand.nextInt(16));
            double d1 = (double)(40 - this.rand.nextInt(24) + this.rand.nextInt(64));
    ...
    protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop)
    {
        ...
        if (state.getBlock() == Blocks.NETHERRACK || state.getBlock() == Blocks.GRAVEL || state.getBlock() == Blocks.SOUL_SAND)
        {
            if (y - 1 < 31)
            {
    ...
    public void generate(World worldIn, int x, int z, ChunkPrimer primer, Random random) {
